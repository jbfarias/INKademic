  ...
  if (setting.type == SettingType::TOGGLE && setting.valuePtr != nullptr) {
    ...
    switch (setting.action) {
      ...
      case SettingAction::QuickActions:
        ...
        break;
      case SettingAction::ManageAnnotationTags:
        if (auto manager = makeUniqueNoThrow<AnnotationTagManagerActivity>(renderer, mappedInput)) {
          startActivityForResult(std::move(manager), resultHandler);
        } else {
          LOG_ERR("SET", "OOM allocating annotation tag manager");
        }
        break;
      case SettingAction::ReaderFontOptions:
      case SettingAction::ReaderPageLayout:
