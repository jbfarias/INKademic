...
void BaseTheme::drawStatusBar(GfxRenderer& renderer, const float bookProgress, const int currentPage,
  ...
  static constexpr int tagIconW = 9;
  static constexpr int tagIconH = 11;
  ...
  const bool showTag = showStatusBarTextLane && isPageTagged;
  const int tagTotalWidth = showTag ? (tagIconW + bmIconGap) : 0;
  const int bmTotalWidth = tagTotalWidth + (showBookmark ? (bmIconW + bmIconGap) : 0);

  if (showTag) {
    const int tagX = leftClusterX;
    const int tagY = textY + (metrics.batteryHeight - tagIconH) / 2 + 5;
    renderer.fillRect(tagX, tagY, tagIconW, tagIconH, foregroundBlack);
    renderer.fillRect(tagX + 2, tagY + 2, tagIconW - 4, tagIconH - 4, !foregroundBlack);
    renderer.fillRect(tagX + tagIconW - 2, tagY + tagIconH / 2, 2, 2, foregroundBlack);
  }

  if (showBookmark) {
    const int bmX = leftClusterX + tagTotalWidth;
