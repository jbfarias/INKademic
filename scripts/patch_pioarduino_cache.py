"""Keep pioarduino's custom SDK cache separate per chip and configuration.

pioarduino historically used one project-level sdkconfig marker. Switching
between C3 and S3 environments, or between the Sticky and X4 Pro S3 profiles,
could therefore trigger an unnecessary rebuild or reuse the wrong SDK.
"""
...
if original_sdkconfig.is_file():
    cached_request = request_file.read_text(encoding="utf-8") if request_file.is_file() else ""
    if cache_matches(requested, cached_request):
        marker = "# TASMOTA__" + hashlib.md5((requested.strip() + mcu).encode("utf-8")).hexdigest()[:16]
        defaults = Path(env.subst("$PROJECT_DIR")) / "sdkconfig.defaults"
        lines = defaults.read_text(encoding="utf-8").splitlines() if defaults.exists() else []
