...
INKademic normally tries to render EPUBs with the full INKademic renderer. Some
...
asking INKademic to simplify the layout work for that book.
...
| ---------------- | ------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| INKademic Default | Most books                                                                                       | Full INKademic styling, detailed CSS, table layout, image sizing, publisher spacing, Bionic Reading, Guide Dots | Nothing by default                                                                            |
| Balanced         | Books where INKademic has fallen back automatically, or books you want to start in a lighter mode | Publisher spacing, image sizing, decorations, Bionic Reading, Guide Dots                                       | Complex CSS lookups and table layout                                                          |
...
Most users can leave books on **INKademic Default**. If a book runs out of memory
while building a section, INKademic will automatically try **Balanced** and then
**Light**. Manual mode changes are mainly for choosing a lighter mode before
opening a known-problem book, or for changing the saved mode after INKademic has
already fallen back.

## INKademic Default

INKademic Default is the normal renderer.

It tries to preserve the book's layout and publisher styling as much as
INKademic supports, including:
...
Use Balanced when INKademic has already fallen back to it, or when you want a
...
Use Light when INKademic has already fallen back to it, or when you want a
known-problem book to start in the safest mode immediately. Light may look less
like the publisher's original layout, but it gives INKademic the best chance of
...
If INKademic cannot build a section in Light because memory is still too tight,
...
When Safe Mode succeeds, INKademic saves those settings for that book. The reader
briefly shows **Safe Mode** over the current page. Your other books keep their
own reader settings.

## Automatic Fallback

If a book starts in INKademic Default and a section runs out of memory while
building, INKademic automatically retries in this order:
...
If the book starts in Balanced, INKademic can fall back to Light and then Safe
Mode. If the book starts in Light, INKademic can still try Safe Mode.

When a fallback succeeds, INKademic saves that successful mode for the book. The
...
INKademic already falls back automatically, but it is useful when you know a book
